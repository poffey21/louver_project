...
# Another option...
# https://github.com/DrRowland/pifi2wifi/blob/master/pifi2wifi.sh
# http://www.0xf8.org/2016/02/using-your-raspberry-pi-zeros-usb-wifi-adapter-as-both-wifi-client-and-access-point/
...
function stop_wifi_ap {
    ...
    ### disable routing everything to the flask site
    iptables -t nat -D PREROUTING -p tcp --dport 80 -j DNAT --to-destination localhost:80 2>/dev/null
    iptables -t nat -D PREROUTING -p tcp --dport 443 -j DNAT --to-destination localhost:80 2>/dev/null
...
function start_wifi_ap {
    ...
    ### route everything to the flask site.
